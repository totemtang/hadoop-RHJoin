...
public class BroadcastJoin extends Configured implements Tool {
	...
	private String hdfsDir = "/tmp/braodcastjoin";
	...
	static class JoinMapper implements
			Mapper<Object, Text, NullWritable, Text>{
		
		private static final Log LOG = LogFactory.getLog(JoinMapper.class.getName());
			...
			if(ht == null){
				...
				try {
					...
					if(htPath != null){
						...
						ht = new HashTable(job, htPath.toString() + "/" + fileName);
					}
				} catch (IOException e) {
		...
		public void map(Object key, Text value,
			...
			Integer probeKey;
			String probeVal;
			if(!ht.isShift()){//hash table is from customer table, read orders' format
				...
				if(ht.containsKey(probeKey)){
					String val = ht.getFromCust(probeKey);
					output.collect(NullWritable.get(), new Text(probeKey.toString() + "|" + val + "|" + probeVal));
				}
			}else{// hash table is from orders table, read customer's format
				...
				probeVal = valString.substring(index1 + 1, index2);
				if(ht.containsKey(probeKey)){
					List<String> valList = ht.getFromOrders(probeKey);
	...
	public static void main(String args[]) throws Exception{
		int res = ToolRunner.run(new Configuration(), new BroadcastJoin(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		if(args.length != 4){
			System.err.println("Usage: <customer> <orders> <output dir> <localDir>");
			System.exit(2);
		}
		JobConf job = new JobConf(getConf(), BroadcastJoin.class);

...
public class DirectedJoinSkew extends Configured implements Tool{
	...
	static class DirectedMapperSkew implements Mapper<Object, Text, NullWritable, Text>{
		...
		public void map(Object key, Text value,
			...
			if(isContain){
				...
				Text outL = new Text(valString.substring(index1 + 1, index2));
				
				Text outR = hashMap.get(probeKey);
				output.collect(NullWritable.get(), 
							new Text(probeKey.toString() + "|" + outR + "|" + outL));
			}
		}
	}
	...
	public int run(String[] args) throws Exception {
		// TODO Auto-generated method stub
		if(args.length != 3){
			System.err.println("Usage: <RTable path> <LTable path> <output path>");
			System.exit(2);
		}
		JobConf job = new JobConf(getConf(), DirectedJoinSkew.class);
		job.setJobName("DirectedJoinSkew");
		Path inputPath = new Path(args[1]);
		Path outputPath = new Path(args[2]);
		job.set("map.dfs.input", args[0]);
		
		//launch map join job
		job.setJarByClass(DirectedJoinSkew.class);
		job.setMapperClass(DirectedMapperSkew.class);
	...
	public static void main(String args[]) throws Exception{
		int res = ToolRunner.run(new Configuration(), new DirectedJoinSkew(), args);
		System.exit(res);
	}
}

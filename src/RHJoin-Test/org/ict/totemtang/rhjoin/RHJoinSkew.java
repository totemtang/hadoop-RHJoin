...
public class RHJoinSkew extends Configured implements Tool {
	...
	public static class LMapper implements
	...
	public static class RReducer implements
		...
		public void reduce(IntWritable key, Iterator<Text> values,
			...
			Text outL;
			if(isContain){
				Text outR = hashMap.get(key);
				while(values.hasNext()){
					outL = values.next();
					context.collect(NullWritable.get(), 
							new Text(key.toString() + "|" + outR + "|" + outL));
				}
			}
		}
	}
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new RHJoinSkew(), args);
		System.exit(res);
	}
	...
	public int run(String[] args) throws Exception {
		...
		JobConf job = new JobConf(getConf(), RHJoinSkew.class);
		job.setJobName("RHJoinSkew");
		job.setBoolean("mapred.nosort.on", true);
		reduceInput = args[1];
		job.set("mapred.reduce.input", reduceInput);
		job.setJarByClass(RHJoinSkew.class);

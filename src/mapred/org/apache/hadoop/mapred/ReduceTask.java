...
class ReduceTask extends Task {
  ...
  public void runNoSort(JobConf job, final TaskUmbilicalProtocol umbilical)
	  ...
	  if(!isLocal){
		  ...
		  if(!reduceCopier.reduceNoSort()){
			  if(reduceCopier.reduceThrowable instanceof FSError){
				  throw (FSError)reduceCopier.reduceThrowable;
			  }
			  throw new IOException("Task: " + getTaskID() +
					  " - The reduce copierAndReducer failed", reduceCopier.reduceThrowable);
		  }
	  }else{
		  LOG.info("Local Mode is not supported by NoSort Version");
		  return;
	  }
  ...
  public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
    throws IOException, InterruptedException, ClassNotFoundException {
	 noSort = job.getBoolean("mapred.nosort.on", false);
	 //boolean noSort = true;
	 if(noSort){
		 runNoSort(job, umbilical);
		 return;
	 }
  ...
  class ReduceCopier<K, V> implements MRConstants {
        ...
        for (TaskCompletionEvent event : events) {
          switch (event.getTaskStatus()) {
            ...
            case TIPFAILED:
            {
              ...
              if(noSort)
            	  reducedMapOutputs.add(event.getTaskAttemptId().getTaskID());
    ...
    //Added By TotemTang
    private volatile Throwable reduceThrowable = null;
    
    private Set <TaskID> reducedMapOutputs = 
    	      Collections.synchronizedSet(new TreeSet<TaskID>());
    
    private List<OutputCopierAndReducer> copierAndReducers = null;
    
    //MapOutputReducer : Copy and Reduce map output
    private class OutputCopierAndReducer extends Thread {
    	JobConf job;
    	...
    	private  ReduceTask.ReduceCopier.MapOutput currentOutput;
    	
    	//QuickReducer
    	QuickReducer quickReducer = null;
    	
    	public OutputCopierAndReducer(JobConf job, Reporter reporter, QuickReducer quickReducer){
    		setName("MapOutputReducer " + reduceTask.getTaskID() + "." + id);
    		LOG.debug(getName() + " created");
    		this.reporter = reporter;
    		...
    		this.quickReducer = quickReducer;
    		this.job = job;
    	}
    	
    	private void start(MapOutputLocation loc){
    		currentLocation = loc;
    	}
    	
    	private void finish(long size) {
    		if(currentLocation != null) {
    			LOG.debug(getName() + "finishing " + currentLocation + " =" + size);
    			synchronized (copyResults) {
    				copyResults.add(new CopyResult(currentLocation, size));
    	...
    	/* Loop forever. Fetch map outputs and  
    	 * 
    	 * */
    	public void run(){
    		while(true){
    			try{
    				...
    				try{
    					...
    					shuffleClientMetrics.failedFetch();
    					size = -1;
    					currentOutput = null; // for security
    				}finally{    					
    					finish(size);
    				}
					if(size >= 0){
						//have copied map-output successfully
						quickReduce(size);
						noteReduced(loc.getTaskId());
					}
    			}catch(InterruptedException e){
    				break; //ALL DONE
    			}catch(FSError e){
    	...
    	/*copy a map output from remote host.
    	 * copy result will stay in memory or on disk 
    	 * */
    	private long copyOutput(MapOutputLocation loc
    								)throws IOException, InterruptedException {
    		//check if we still need to copy the output from this location
    	...
    	/* Get the map output info a local file or in memory from remote server
    	 * */
    	private MapOutput getMapOutput(MapOutputLocation mapOutputLoc,
    										Path filename, int reduce)
    	throws IOException, InterruptedException {
        ...
        /*Save the map taskid whose output have been reduced
         * */
        private void noteReduced(TaskID taskId){
        	reducedMapOutputs.add(taskId);
        	synchronized(reducedMapOutputs){
        	if(reducedMapOutputs.size() >= numMaps)
        		reducedMapOutputs.notify();
        	}
        }
        
        private synchronized void saveMapOutput(MapOutput output){
        	//Sanity Check
        	if(output == null )
        		currentOutput = null;
        	else if (output.compressedSize <= 0)
        		currentOutput = null;
        	currentOutput = output;
        	
        }
        
        private void quickReduce(long size) throws InterruptedException{
        	try{
        		quickReducer.quickReduce(currentOutput, size);
        	}catch (IOException ioe) {
        		LOG.info("TotemTang : Quick Reduce error, try to quit");
        		if(reduceThrowable != null)
        			reduceThrowable = ioe;
        		try {
					quickReducer.close();
				} catch (IOException e) {
					// TODO Auto-generated catch block
					LOG.info("TotemTang : " + e);
				}
        	}
        }
    	
    }// MapOutputReducer END
    
    private class QuickReducer<INKEY, INVALUE, OUTKEY, OUTVALUE>{
    	JobConf job;
    	Reducer<INKEY, INVALUE, OUTKEY, OUTVALUE> reducer;
    	String finalName;
    	FileSystem fs;
    	...
    	public QuickReducer(JobConf job) throws IOException{
    		this.job = job;
    		reducer = ReflectionUtils.newInstance(job.getReducerClass(), job);
    		finalName = getOutputName(getPartition());
		...
		public synchronized void quickReduce(MapOutput mapOutput, long size) throws IOException, InterruptedException{
    		if(mapOutput == null)
        		return ;
        	else if(size == 0){
        		mapOutput = null;
        	...
        	if(mapOutput.compressedSize <= 0 ){
        		reduceThrowable  = new Exception("Map-Output's size is not bigger than zero in quickReduce");
        		return;
        	}
        	
        	if(reducer == null || out == null)
        		return ;
    		
        	OneSegment<K, V> segIn = null;
        	
        	
        	if(mapOutput.inMemory){
        		Reader<K, V> reader = 
        				new InMemoryReader<K, V>(ramManager, mapOutput.mapAttemptId,
        											mapOutput.data, 0, mapOutput.data.length);
        		segIn = new OneSegment<K, V>(reader, true);
        	}else{
        		segIn = new OneSegment<K, V>(job, rfs, mapOutput.file,
        										codec, job.getKeepFailedTaskFiles());
        		segIn.init(spilledRecordsCounter);
        	}
        	
        	Class<INKEY> keyClass = (Class<INKEY>) job.getMapOutputKeyClass();
        	Class<INVALUE> valueClass = (Class<INVALUE>) job.getMapOutputValueClass();
        	
        	SegmentResolver<INKEY, INVALUE> keyvalue = 
        			new SegmentResolver<INKEY, INVALUE>(segIn, keyClass,
        					valueClass, job, reporter);
        	
        	keyvalue.informReduceProgress();
        	while(keyvalue.nextKey()){
        		reduceInputKeyCounter.increment(1);
        		reducer.reduce(keyvalue.getKey(), keyvalue, collector, reporter);
        		keyvalue.informReduceProgress();
        	}        	
        	segIn.close();
    	}
        ...
        public class OneSegment<K extends Object, V extends Object>{
        	...
        	public OneSegment(Reader<K, V>reader, boolean preserve){
        		this.reader = reader;
        	...
        	public OneSegment(Configuration conf, FileSystem fs, Path file,
        ...
        private class SegmentResolver<KEY,VALUE> implements Iterator<VALUE>{
        	OneSegment<K, V> segIn;
        	...
        	public SegmentResolver (OneSegment<K, V> segIn,
        								Class<KEY> keyClass,
        	...
        	throws IOException {
        		...
        		this.valDeserializer.open(valueIn);
        	}
        	...
        	public boolean nextKey() throws IOException {
        		if(segIn.next()){
        			DataInputBuffer nextKey = segIn.getKey();
        			keyIn.reset(nextKey.getData(), nextKey.getPosition(), nextKey.getLength());
        			key = keyDeserializer.deserialize(key);
        			
        			DataInputBuffer nextValue = segIn.getValue();
        			valueIn.reset(nextValue.getData(), nextValue.getPosition(), nextValue.getLength());
    ...
    public boolean reduceNoSort() throws IOException{
    	int totalFailures = 0;
    	int numInFlight = 0, numCopied = 0;
    	DecimalFormat mbpsFormat = new DecimalFormat("0.00");
    	final Progress copyPhase = reduceTask.getProgress().phase();
    	GetMapEventsThread getMapEventsThread = null;
    	QuickReducer quickReducer = new QuickReducer(conf);
    	
    	LOG.info("TotemTang: nosort on");
    	...
    	copierAndReducers = new ArrayList<OutputCopierAndReducer>(numCopiers);
    	
    	//start all the copying threads
    	for (int i = 0; i < numCopiers; i++){
    		OutputCopierAndReducer copierAndReducer = new OutputCopierAndReducer(conf, reporter, quickReducer);
    		copierAndReducers.add(copierAndReducer);
    		copierAndReducer.start();
    	}
    	...
    	//loop until we get all required map outputs and have handled them
    	while(copiedMapOutputs.size() < numMaps &&    			 
    			reduceThrowable == null) {
    		currentTime = System.currentTimeMillis();
    		boolean logNow = false;
    		if(currentTime - lastOutputTime > MIN_LOG_TIME){
    			lastOutputTime = currentTime;
    		...
    		while(locItr.hasNext()) {
    			MapOutputLocation loc = locItr.next();
    			List<MapOutputLocation> locList = mapLocations.get(loc.getHost());
    		...
    		synchronized(scheduledCopies) {
    			...
    			while(hostsItr.hasNext()) {
    				...
    				List<MapOutputLocation> knownOutputsByLoc = mapLocations.get(host);
    				...
    				synchronized(knownOutputsByLoc){
    					...
    					while(locItr.hasNext()){
    						...
    						//Do not schedule fetched from OBSOLETE maps
    						if(obsoleteMapIds.contains(loc.getTaskAttemptId())){
    							locItr.remove();
    							continue;
    						}
    		...
    		while(numInFlight > 0 && reduceThrowable == null){
    			//LOG.debug(reduceTask.getTaskID() + " numInFlight = " +
    			//			numInFlight);
                //the call to getCopyResult will either 
                //1) return immediately with a null or a valid CopyResult object,
    	...
    	//copy done, we need to wait for reduce done
    	synchronized(reducedMapOutputs){
    		while(reducedMapOutputs.size() < numMaps && reduceThrowable == null){
				try {
					reducedMapOutputs.wait();
				} catch (InterruptedException e) {
					// ignore
				}
    		}
    	...
    	synchronized(copierAndReducers){
    		synchronized(scheduledCopies){
    			for(OutputCopierAndReducer cr : copierAndReducers){
    				cr.interrupt();
    			}
    			copierAndReducers.clear();
    		}
    	}
    	...
    	return reduceThrowable == null && copiedMapOutputs.size() == numMaps && reducedMapOutputs.size() == numMaps;
    }    
